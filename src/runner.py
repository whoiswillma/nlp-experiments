...
def main(args):
  ...
  train_data = Conll2003(examples=train['tokens'][:1000], labels=train['ner_tags'][:1000], ner_tags=ner_tags, device=device)
  # ISSUE: val data has its own idx_to_tokens --> need to fix
  # FIX: make idx2tags and tags2idx before and pass in as param to dset
