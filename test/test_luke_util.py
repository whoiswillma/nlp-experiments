...
from luke_util import (
    ...
    greedy_extract_named_entity_spans,
    take_closure_over_entity_spans,
)
...
class LukeUtilTest(unittest.TestCase):
    def test_chunked(self):
        ...
        self.assertEqual(chunked("ABCDEF", 3), ["ABC", "DEF"])

        self.assertEqual(chunked([], 1), [])

        self.assertEqual(chunked("A", 3), ["A"])

    def test_take_closure_over_entity_spans_to_labels(self):
        entity_spans = {(0, 1): 500}
        closure = take_closure_over_entity_spans_to_labels(entity_spans)
        expected = {(0, 1): 500}
        self.assertEqual(closure, expected)

        entity_spans = {(2, 4): 500}
        closure = take_closure_over_entity_spans_to_labels(entity_spans)
        expected = {(2, 3): 500, (3, 4): 500, (2, 4): 500}
        self.assertEqual(closure, expected)

        entity_spans = {(2, 4): 500, (6, 7): 501}
        closure = take_closure_over_entity_spans_to_labels(entity_spans)
        expected = {(2, 3): 500, (3, 4): 500, (2, 4): 500, (6, 7): 501}
        self.assertEqual(closure, expected)

        closure = {(2, 3): 500, (3, 4): 500, (2, 4): 500, (6, 7): 501}
        expected = closure
        self.assertEqual(closure, expected)

    def test_take_closure_over_entity_spans(self):
        self.assertEqual(
            {(0, 1), (0, 2), (1, 2)}, take_closure_over_entity_spans([(0, 2)])
        )

        self.assertEqual(
            {(3, 4), (5, 6)}, take_closure_over_entity_spans([(3, 4), (5, 6)])
        )
    ...
    def test_greedy_extract_named_entity_spans_selects_entity(self):
        ...
        self.assertEqual(
            {500: [(1, 1)]}, greedy_extract_named_entity_spans(span_label_logit, 0)
        )
    ...
    def test_greedy_extract_named_entity_spans_works_with_only_nonentity(self):
        ...
        self.assertEqual({}, greedy_extract_named_entity_spans(span_label_logit, 0))
    ...
    def test_greedy_extract_named_entity_spans_selects_entity_by_decreasing_logit(self):
        ...
        self.assertEqual(
            {501: [(1, 1)]}, greedy_extract_named_entity_spans(span_label_logit, 0)
        )
