...
from ner import (
    ...
    compute_binary_confusion_matrix_from_batched_bio, extract_named_entity_spans_from_chunks
)
...
class TestNerUtil(unittest.TestCase):
    ...
    def test_extract_named_entity_spans_from_chunks(self):
        self.assertEqual(
            {'PER': [(1, 1)]},
            extract_named_entity_spans_from_chunks(['O', 'PER', 'O'])
        )

        self.assertEqual(
            {'PER': [(0, 0)]},
            extract_named_entity_spans_from_chunks(['PER'])
        )

        self.assertEqual(
            {'PER': [(0, 1)]},
            extract_named_entity_spans_from_chunks(['PER', 'PER'])
        )
